defmodule Absinthe.GraphqlWS.Transport do
  ...
  * fallthrough - If `c:Absinthe.GraphqlWs.Socket.handle_message/2` is defined on the socket,
    ...
    case run(query, config[:schema], config[:pipeline], opts) do
      ...
      {:more, %{"subscribed" => topic}, continuations, context} ->
        ...
        socket =
          ...
          |> then(&%{&1 | subscriptions: Map.put(&1.subscriptions, topic, id)})
          |> handle_subscription_continuation(continuations)

        response =
          Enum.map(
            socket.continuation_accumulator,
            &{:text, Message.Next.new(id, &1.data)}
          )
      ...
      {:more, %{data: _} = _reply, _continuations, _context} ->
        # TODO
        {:reply, :error,
         {:text, Message.Error.new(id, "Non-subscription continuation not implemented")}}
  ...
  defp handle_subscription_continuation(socket, continuations) do
    case Absinthe.Pipeline.continue(continuations) do
      ...
      {:ok, %{result: result}, _phases} ->
        ...
        case result[:continuations] do
          nil -> socket
          c -> handle_subscription_continuation(socket, c)
        end
    end
  end
